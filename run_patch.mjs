...
const tOldAdd = `async addTransaction(user: User, patientId: string, data: any) {`;

if (!content.includes(tOldAdd)) {
  console.log('Not found');
  process.exit(1);
}

const startInd = content.indexOf(tOldAdd);
const endIndStr = `await this.saveToStorage();\n        return updatedTransactions[tIdx];\n    }`;
const endInd = content.indexOf(endIndStr, startInd) + endIndStr.length;
...
const fin = content.substring(0, startInd) + newCode + content.substring(endInd);
fs.writeFileSync(file, fin);
console.log('Done!');
